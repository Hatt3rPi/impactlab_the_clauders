---
title: Inicio
description: Wiki interna del equipo The Clauders para el Claude Impact Lab Chile 2026.
hide:
  - navigation
---

# The Clauders — Wiki del equipo

> *"5 millones de chilenos tienen derechos financieros que no logran entender."*
> — Claude Impact Lab Chile 2026

Bienvenidos al espacio de trabajo del equipo **The Clauders** para el [Claude Impact Lab Chile 2026 — *El puente al ciudadano*](https://fintech.benditaia.cl/es), 48 horas de hackathon organizado por **Anthropic, BenditaIA y FinteChile** los días **6 y 7 de mayo de 2026** en Espacio Riesco, dentro del Chile Fintech Forum.

---

## Resumen ejecutivo

| | |
|---|---|
| **Competencia** | Claude Impact Lab Chile 2026 — El puente al ciudadano |
| **Organizador** | Anthropic + BenditaIA + FinteChile |
| **Fechas del lab** | 6-7 de mayo de 2026 (48 h) |
| **Lugar** | Espacio Riesco, Santiago — Chile Fintech Forum 2026 |
| **Líneas temáticas** | Inclusión financiera · Ciberseguridad ciudadana · Protección de datos |
| **Modalidad** | Equipos de 2-4 personas (también solo) |
| **Stack obligatorio** | Claude API, Claude Code, Agent SDK, MCPs |
| **Datasets** | CMF, SII, SERNAC (datos públicos) |
| **Pitch final** | 7 de mayo a las 12:00 |

---

## Estado actual del equipo

!!! info "Fase: Exploración"
    Estamos evaluando las **3 líneas temáticas en paralelo**. La decisión se tomará en la próxima reunión usando la [matriz comparativa](lineas-tematicas/index.md).

- [x] Inscripción individual completada
- [ ] Formación de equipo (límite **5 de mayo**)
- [ ] Selección de línea temática
- [ ] Definición de problema y propuesta
- [ ] Prototipo funcional
- [ ] Pitch (7 de mayo · 12:00)

---

## Próximos hitos

| Fecha | Hito |
|---|---|
| **30 abr 2026** | :material-alert: Cierre de inscripción individual (23:59) |
| **5 may 2026** | :material-account-group: Cierre de formación de equipos |
| **6 may 2026** | :material-rocket-launch: Inicio del lab + keynote |
| **7 may 2026 · 12:00** | :material-presentation: Pitch final |
| **junio 2026+** | :material-trending-up: Acelera AI Fintech Sandbox (60 días, equipos ganadores) |

Ver el [timeline completo](competencia/timeline.md).

---

## Atajos

<div class="grid cards" markdown>

-   :material-trophy: **Competencia**

    ---

    Información oficial: criterios, deliverables, premios.

    [:octicons-arrow-right-24: Ir a competencia](competencia/index.md)

-   :material-lightbulb-on: **Ideas**

    ---

    Tablero de ideas con scoring contra los 5 criterios.

    [:octicons-arrow-right-24: Ver tablero](ideas/index.md)

-   :material-magnify: **Research**

    ---

    Notas de investigación regulatoria, de usuarios, mercado y tecnología.

    [:octicons-arrow-right-24: Explorar research](research/index.md)

-   :material-stack-overflow: **Stack técnico**

    ---

    Claude API, Agent SDK, MCPs, datasets de CMF/SII/SERNAC.

    [:octicons-arrow-right-24: Ver stack](stack/index.md)

-   :material-account-multiple: **Reuniones**

    ---

    Transcripciones y resúmenes ejecutivos.

    [:octicons-arrow-right-24: Ver reuniones](reuniones/index.md)

-   :material-file-tree: **Especificaciones**

    ---

    Arquitectura, alcance del prototipo y decisiones (ADRs).

    [:octicons-arrow-right-24: Ver especificaciones](especificaciones/index.md)

</div>
